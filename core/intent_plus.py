...
DIRECT_EXPENSE_PATTERNS = [
    re.compile(
        ...
        r"(?:花了|花|消费了|消费|付了|付款|支出|用了)\s*"
        r"(?P<amount>\d+(?:\.\d+)?)\s*"
        r"(?:元|块|块钱|rmb|RMB|￥)\s*"
        r"(?:买了|买|购买了|购入|入手了|用于)?\s*"
        r"(?P<item>.+)"
    ),
    re.compile(
        r"(?:我|俺|刚刚|刚才|今天|昨天|已经)?\s*"
        r"(?:买了|购买了|购入|入手了|花了|花|消费了|支出|扣款|记账|记一下)\s*"
        r"(?P<item>.*?)\s*"
        r"(?:已经|一共|总共|花了|花|用了|消费了|支出|付款|扣款)?\s*"
        r"(?P<amount>\d+(?:\.\d+)?)\s*"
        r"(?:元|块|rmb|RMB|￥)"
    ),
    re.compile(
...
def _clean_item_name(item: str) -> str:
    item = item or ""
    item = re.sub(
        r"^(我|俺|刚刚|刚才|今天|昨天|已经|买了|购买了|购入|入手了|花了|花|消费了|支出|扣款|记账|记一下|买)+",
        "",
        item,
    )
    item = re.sub(r"(花了|花|用了|消费了|支出|付款|扣款|价格|金额|一共|总共)$", "", item)
    item = item.strip(" ，。！？；：,.!?;:\"'`()[]{}")
    return item[:40]
